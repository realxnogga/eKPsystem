...
if (!empty($data['image'])) {
    ...
    if (file_put_contents($filename, $imageData)) {
        ...
        $countrowquery = "SELECT COUNT(*) FROM user_flowchart WHERE user_id = :user_id";
        ...
        if ($rowCount === 0) {
            $insertStmt = $conn->prepare("INSERT INTO user_flowchart (user_id, flowchart_image) VALUES (:user_id, :flowchart_image)");
            $insertStmt->bindParam(':user_id', $userId, PDO::PARAM_INT);
            $insertStmt->bindParam(':flowchart_image', $imagename, PDO::PARAM_STR);
            $insertStmt->execute();
        } else {
            $updateStmt = $conn->prepare("UPDATE user_flowchart SET flowchart_image = :flowchart_image WHERE user_id = :user_id");
